...
three$centralityDummy_CoronaPandemie <- NULL 


three$num_edges_invaliddashedpercent <- three$num_edges_invaliddashed / three$num_edges_dashed
three$num_edges_invaliddashedpercent[three$num_edges_invaliddashedpercent == 0] <- NA

three$dummy_edges_invaliddashedpercen <- ifelse(is.na(three$num_edges_invaliddashedpercent), yes = 0, no = 1)


# constant, data entry error
# or
...
table_whyisdashedhigh_mean <- three %>%
...
# table_whyisdashedhigh_mean %>% stargazer(out = "bb.html", type = "html", summary = FALSE)
  ...
  table_whyisdashedhigh_sd <- three %>%
 group_by(group, dummy_edges_invaliddashedpercen) %>%
  select(!(CAM_ID:assortativity_valence:assortativityDegree)) %>%
...
linearer <- lm(num_edges ~ num_nodes, data=three)

summary(linearer)

plot(three$num_edges, three$num_nodes, xlab="Predictor", ylab="Outcome", col="darkblue", pch=16, main="Simple Linear Regression")

for (i in 1:length(linearer$fitted.values))
  lines(c(three$num_edges[i], three$num_edges[i]), c(three$num_nodes[i], linearer$fitted.values[i]), col="blue")

abline(linearer, col="darkred")

scatter.smooth(x=three$num_edges, y=three$num_nodes, main="Edges ~ Nodes") 
...
# psych::cor.plot(r = cor(three[, 2:23], use = "pairwise.complete.obs"), upper = FALSE, xlas = 2)
# dev.off()
psych::cor.plot(r = cor(three[, c(2:24, 26)], use = "pairwise.complete.obs"), upper = FALSE, xlas = 2)
plot(three$num_edges_invaliddashedpercent, three$assortativity_valence)
```
